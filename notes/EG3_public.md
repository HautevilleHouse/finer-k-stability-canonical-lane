...
In-paper anchor: `paper/FINER_K_STABILITY_PREPRINT.md` (`FKS_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of refined K-stability and test-configuration control through an admissible stability-refinement closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`FKS_G3` closes when `kappa_compact` provides the compactness/no-collapse control: normalized near-failure families stay precompact and restart spacing does not collapse.
This is the first-failure capture contribution to `M_FKS`.
...
If near-failure extraction and no-Zeno spacing hold, then `FKS_G3` prevents hidden drift before rigidity is applied.
